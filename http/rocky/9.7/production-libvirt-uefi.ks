...
sshkey --username=vagrant "ssh-rsa AAAA...."

url --url="https://download.rockylinux.org/pub/rocky/9/BaseOS/x86_64/kickstart/"

# Disk setup
# Bootloader setup (UEFI system)
bootloader --timeout=1 --append="console=tty0 console=ttyS0,115200n8 no_timer_check crashkernel=auto net.ifnames=0" --boot-drive=vda
...
part /boot --fstype="xfs" --size=1024 --asprimary --ondrive=vda

part /boot/efi --fstype="efi" --ondisk=vda --size=600 --fsoptions="umask=0077,shortname=winnt"
part pv.1 --size 1 --grow --ondrive=vda

volgroup system --pesize=32768 pv.1
logvol swap  --fstype=swap --name=lv_swap  --vgname=system --size=2048
logvol /     --fstype="xfs"  --name=lv_root  --vgname=system --size=8192
logvol /var  --fstype="xfs"  --name=lv_var   --vgname=system --size=8192
logvol /home --fstype="xfs"  --name=lv_home  --vgname=system --size=2048
logvol /opt  --fstype="xfs"  --name=lv_opt   --vgname=system --size=4096
logvol /tmp  --fstype="xfs"  --name=lv_tmp   --vgname=system --size=2048
logvol /srv  --fstype="xfs"  --name=lv_srv   --vgname=system --size=2048
logvol /var/log --fstype="xfs" --name=lv_log --vgname=system --size=4096
logvol /var/tmp --fstype="xfs" --name=lv_vartmp --vgname=system --size=2048

# Root gets the remaining space
logvol /usr  --fstype="xfs"  --name=lv_usr   --vgname=system --grow --size=2048
...
dnf -y install perl ansible sudo wget
...
# This disables persistent PV/VG locking, which is safe and recommended for golden images or anything being cloned.
rm -f /etc/lvm/devices/system.devices
lvmconf --disable-devicesfile
